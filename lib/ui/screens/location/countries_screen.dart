...
class CountriesScreenState extends State<CountriesScreen> {
  ...
  void initState() {
    ...
    defaultLocation();
    _checkAndNavigateToCyprus(context.read<FetchCountriesCubit>().state
            is FetchCountriesSuccess
        ? (context.read<FetchCountriesCubit>().state as FetchCountriesSuccess)
            .countriesModel
        : []);
  }

  void _checkAndNavigateToCyprus(List<CountriesModel> countriesModel) {
    final cyprus = countriesModel.firstWhere(
      (country) => country.name!.toLowerCase() == "cyprus",
      orElse: () => CountriesModel(),
    );

    if (cyprus.name != null) {
      print(
          "Country ID: ${cyprus.id} Country Name: ${cyprus.name} From: ${widget.from}");
      Navigator.pushNamed(context, Routes.statesScreen, arguments: {
        "countryId": cyprus.id!,
        "countryName": cyprus.name!,
        "from": widget.from
      });
    } else {
      print("Cyprus not found");
    }
  }
                        ...
                        child: ListView.separated(
                          ...
                          itemBuilder: (context, index) {
                            ...
                            return ListTile(
                              onTap: () {
                                print(
                                    "Country ID: ${country.id} Country Name: ${country.name} From: ${widget.from}");
                                Navigator.pushNamed(
